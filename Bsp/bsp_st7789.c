...
{
    //调用spi1总线发送一个数据
    ROM_SPI_transmitData(EUSCI_B2_BASE,dat);
}
...
void Lcd_Init(void)
{
    GPIO_setAsOutputPin(OLED_DC_GPIO_Port,OLED_DC_Pin);
	GPIO_setAsOutputPin(OLED_RST_GPIO_Port,OLED_RST_Pin);
    GPIO_setAsOutputPin(OLED_BLK_GPIO_Port,OLED_BLK_Pin);
